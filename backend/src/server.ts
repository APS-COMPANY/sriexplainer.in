...
const formatSeries = (row: any) => {
  ...
  // Dynamically calculate latest uploaded episode number & actual video quality from real production database
  let episodeCount = 0;
  ...
  try {
    ...
    if (episodeCount > 0) {
      ...
      const rawQ = (latestEp.quality || "1080P").toUpperCase().trim();
      latestEpisodeQuality = rawQ || "1080P";
    }
  } catch (err) {
