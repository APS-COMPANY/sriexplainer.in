...
export async function GET() {
  try {
    ...
    let seriesRows: any[] = [];
    try {
      seriesRows = await tursoQuery("SELECT * FROM series WHERE LOWER(status) = 'upcoming' OR isUpcoming = 1 ORDER BY createdAt DESC LIMIT 20");
    } catch {
      try {
        seriesRows = await tursoQuery("SELECT * FROM series WHERE LOWER(status) = 'upcoming' ORDER BY createdAt DESC LIMIT 20");
      } catch {
        seriesRows = [];
      }
    }
    ...
    try {
      episodeRows = await tursoQuery(`
        ...
        WHERE (e.scheduledReleaseAt IS NOT NULL AND e.scheduledReleaseAt > ?)
